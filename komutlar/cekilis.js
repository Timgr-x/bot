            ...
            msg.edit('🔸 | **Şimdi de ödülü yaz bakalım**').then(msg => {
              message.channel.awaitMessages(filter, {
                ...
                try {
                  ...
                  .setColor("#f558c9")
                  .setDescription(`**Ödül: ${title}** \n🎉'emojisine Basarak Katıl \nKalan Süre : ${duration} \n **Başlama Zamanı :** ${hours}:${minutes}:${seconds} ${suffix}`)
                  .setFooter(message.author.username + " (Hemera Bot çekiliş sistemi)", message.author.avatarURL);
                  message.guild.channels.find("name" , room).send(' ✔️ **ÇEKİLİŞ BAŞLADI** ✔️' , {embed: giveEmbed}).then(m => {
                     let re = m.react('🎉');
                     setTimeout(() => {
                       ...
                       .setTitle(title)
                       .setColor("#f558c9")
            .setFooter("(Hemera Bot çekiliş sistemi)")
                     ...
                     m.edit('** 🎉 ÇEKİLİŞ BİTTİ 🎉**' , {embed: endEmbed});
                       ...
                       var embedLel = new Discord.RichEmbed()
                        .setColor("#f558c9")
                        .setDescription("Ödülünü Moderatörleri Etiketleyerek Alabilirsin!").setFooter("(Hemera Bot çekiliş sistemi)")
